...
int veml6030_init_config(struct veml6030_data *drv_data)
{
	...
	if (ret) {
		...
		LOG_INF("i2c_burst_write(0x%04X) success",config);
	}
	return ret;
}
...
int veml6030_init(const struct device *dev)
{
	...
	return veml6030_init_config(drv_data);
}
